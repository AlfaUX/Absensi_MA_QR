...
<!-- Content Wrapper -->
  <div class="content-wrapper">
      ...
      <section class="content">
          <div class="container-fluid">
              <a href="<?= base_url('siswa/tambah') ?>" class="btn btn-primary">Tambah Siswa</a>
              <form action="<?= base_url('siswa/import_csv') ?>" method="post" enctype="multipart/form-data" class="mt-3">
                  <input type="file" name="csv_file" accept=".csv" required>
                  <button type="submit" class="btn btn-success">Import CSV</button>
                  <a href="<?= base_url('siswa/download_template') ?>" class="btn btn-primary">Download Template CSV</a>
              </form>
              <table class="table mt-4">
                  ...
                  <tbody>
                      ...
                      <tr>
                          ...
                          <td>
                              <a href="<?= base_url('siswa/edit/'.$row['id']) ?>" class="btn btn-warning btn-sm">
                                  <i class="fas fa-edit"></i>
                              </a>
                              <a href="<?= base_url('siswa/delete/'.$row['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Yakin ingin menghapus?')">
                                  <i class="fas fa-trash"></i>
                              </a>
                              <a href="<?= base_url('siswa/generate_qr/'.$row['id']) ?>" class="btn btn-success btn-sm">
                                  <i class="fas fa-qrcode"></i> Tampilkan QR
                              </a>
                              <a href="<?= base_url('siswa/download_qr/'.$row['id']) ?>" class="btn btn-primary btn-sm">
